...
export class LatencyTester {
  ...
  private async testSingleRpc(chainId: number, url: string): Promise<LatencyTestResult> {
    ...
    if (getCodeResponse?.error) {
      ...
      return { url, latency: Infinity, status, error: redactRpcDiagnostic(errMsg) };
    }
    ...
    if (this.validateChainId) {
      ...
      if (observedChainId !== chainId) {
        ...
        return { url, latency, status, error: redactRpcDiagnostic(errMsg), observedChainId };
      }
    }
    ...
    if (syncingResponse?.result !== false) {
      ...
      return { url, latency, status, error: redactRpcDiagnostic(errMsg) };
    }
