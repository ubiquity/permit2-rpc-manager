...
export class MethodAwareRpcScorer {
  ...
  private calculateScore(
    ...
    if (stats.avgResponseTime > 0) {
      ...
      // Normalize: 1.0 for <500ms, 0.5 for 2s, 0.1 for >5s
      const timeScore = Math.max(0.1, Math.min(1, 2000 / stats.avgResponseTime));
      score *= (0.3 * timeScore);
    ...
    if (stats.lastFailure > stats.lastSuccess && 
        now - stats.lastFailure < 5000) { // Failed in last 5 seconds
      score *= 0.7;
    }
    
    // Boost for consistent recent performance
    if (stats.successCount >= 5 && successRate === 1 && 
        now - stats.lastSuccess < 10000) { // Perfect recent record
      score *= 1.2;
    }
