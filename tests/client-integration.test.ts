...
describe("Client Integration Tests - Failover Simulation", () => {
  ...
  beforeEach(() => {
    ...
    const ongoingTests =
      (manager.rpcSelector as any).constructor.ongoingLatencyTests;
    if (ongoingTests) {
      ongoingTests.clear();
    }
  });
  ...
  it(
    "should successfully complete concurrent calls despite primary RPC failure",
    async () => {
      console.log("\n--- Starting Concurrent Failover Test ---");

      // 1. Get the initial ranked list (runs latency test if needed)
      const initialRankedList = await manager.rpcSelector.getRankedRpcList(
        GNOSIS_CHAIN_ID,
      );
      ...
      manager.executeRpcCall = mock(
        async (url: string, method: string, params: any[]) => {
          console.log(
            `>>> MOCK executeRpcCall: Intercepted call to ${url} for ${method}`,
          );
          if (url === primaryRpc) {
            primaryFailCount++;
            console.log(
              `>>> MOCK executeRpcCall: Simulating failure for primary ${url}`,
            );
      ...
      const promises = addressesToTest.map((addr) =>
        readContract<bigint>({
          ...
          functionName: "balanceOf",
          args: [addr],
          logger: (level, msg, ...params) =>
            console.log(`[ReadContract:${level}] ${msg}`, ...params),
        }).catch((err) => {
      ...
      console.log(
        `Executing ${promises.length} concurrent readContract calls...`,
      );
